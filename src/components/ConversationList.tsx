...
export const ConversationList: React.FC<ConversationListProps> = ({
  ...
  return (
    <div className="relative flex flex-col h-full bg-white rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="p-4 border-b border-[#E8E8E8]">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-[#F7F8FA] border border-[#D9D9D9] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1A1A1A]/10 focus:border-[#999] transition-all"
          />
        </div>
            ...
            {conversations.map((conversation, index) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-full p-4 text-left transition-all duration-200 hover:bg-[#F7F8FA] border-l-3",
                    isSelected
                      ? "bg-[#E8855A]/10 hover:bg-[#E8855A]/15 border-l-[#E8855A]"
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="relative flex-shrink-0">
                      {conversation.isGroup ? (
                        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#F2F2F2] to-[#E8E8E8] flex items-center justify-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-1.5 min-w-0">
                          {conversation.isGroup && (
                            <Users className="w-3.5 h-3.5 text-[#B3B3B3] flex-shrink-0" />
                          )}
                          <h3 className={cn(
                            "font-medium truncate",
                            hasUnread ? "text-[#1A1A1A]" : "text-[#333]"
                          )}>
                        ...
                        <span className="text-xs text-[#B3B3B3] flex-shrink-0 ml-2">
                      ...
                      <p className={cn(
                        ...
                        hasUnread ? "text-[#1A1A1A] font-medium" : "text-[#999]"
                      )}>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {accountAiEnabled && userSettings.preferences.ai.enabled && userSettings.preferences.ai.autoReply && (
                          <span
                            className="flex items-center justify-center w-5 h-5 bg-[#FF6B35] text-white rounded-full"
                            title="AI接管中"
                          >
                        ...
                        {accountAiEnabled && userSettings.preferences.ai.enabled && !userSettings.preferences.ai.autoReply && (
                          <span
                            className="flex items-center justify-center w-5 h-5 bg-[#FFE8DD] text-[#E8855A] rounded-full"
                            title="AI辅助"
                          >
                        ...
                        {conversation.customer.tags.slice(0, 1).map((tag, i) => (
                          <span
                            key={`customer-${i}`}
                            className="px-2 py-0.5 text-xs bg-[#FFF7F3] text-[#E8855A] rounded-full"
                          >
                            {tag}
                          </span>
                        ))}
                        ...
                        {conversation.tags
                          ...
                          <span
                            key={`conv-${i}`}
                            className="px-2 py-0.5 text-xs bg-[#F2F2F2] text-[#666] rounded-full"
                          >
                            {tag}
                          </span>
                        ))}
                        ...
                        {/* Unread Count */}
                        {hasUnread && (
                          <span className="ml-auto px-2 py-0.5 text-xs font-medium bg-[#FF6B35] text-white rounded-full">
                        ...
                        {!hasUnread && conversation.lastMessage?.senderType !== 'customer' && (
                          <CheckCheck className="w-4 h-4 text-[#B3B3B3] ml-auto" />
                        )}
                      </div>
                    </div>
                  </div>
      ...
      {/* Footer Stats */}
      <div className="p-3 border-t border-[#E8E8E8] bg-[#F7F8FA]">
